 ...
 * @class
 * @example
 * var recorder = new GifRecorder(mediaStream || canvas || context, { width: 200, height: 200, frameRate: 200, quality: 10 });
 * recorder.record();
 * recorder.stop(function(blob) {
...
<!--뉴탭 열때 녹화 확인 영상 크기-->
            if (!config.width) {
                config.width = video.offsetWidth || 200;
            }

            if (!this.height) {
                config.height = video.offsetHeight || 200;
            }
